...
| `restoreStandaloneExisting` | Restores the WebWidget captured before Paired mode while Standalone. | `false` replaces any existing WebWidget without retaining it. Before enabling `true`, configure the intended existing WebWidget so its URL, ID, name, and refresh interval are saved during the first valid Standalone capture. The snapshot is durable internal state, not continuously rediscovered. An empty restore record is discarded so capture can retry, but an original WebWidget that RoomOS already removed must be recreated before it can be learned. |
