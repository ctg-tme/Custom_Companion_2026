...
| `weather`, `time`, `Standalone`, and `Paired` fields | Configure widget weather, clock, status copy, and icons. | Use valid coordinates, temperature unit, IANA time zone, text, and reachable HTTPS image URLs. Runtime owns heading, theme, `info1`, `info3`, and `hideSettings`. |
...
| Custom panels | `cc26_access` launches protected `cc26_hidden`; `cc26_error` replaces them during Unhealthy State. | Access and hidden panels remain. Legacy panel `cc26` is removed. | Solution-owned UI, regenerated by runtime. |
