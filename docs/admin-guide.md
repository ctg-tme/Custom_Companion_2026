...
| `UserInterface.WebWidget.CompanionWidget.enabled` | Enables the complete managed Companion WebWidget workflow. | When `false`, initialization removes only `cc26WebWidget` and does not inventory, capture, restore, subscribe to, build, or update WebWidgets. A differently owned WebWidget remains available for another solution. |
