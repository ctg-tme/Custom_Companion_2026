...
Companion Device Deployment Configuration defaults `allowInsecureHTTPS` to `false`. The Parent Room runtime initializes the device-wide HTTPClient setting with the same safe baseline, then selects the request option from the saved configuration for the specific Companion Device serial. `ParentReadyRequest` carries that HTTPClient subset as bootstrap data so the first Parent-to-Companion callback follows the Companion Device's policy before full configuration has been accepted. Device certificates and host names therefore need a trusted chain in both directions unless a Device Administrator deliberately sets `config.httpClient.allowInsecureHTTPS` to `true`, accepting untrusted certificates for requests associated with that Companion Device.
...
- `Config.httpClient.allowInsecureHTTPS`
- `TransactionId`

Companion Device serial, name, and MAC address are not stored in base config. The Companion Device pulls those values from local xAPI at runtime and places them in the message envelope when needed. The bootstrap `Config` contains only the HTTPClient subset required for the Parent Room Device to send `ParentReady`; the complete Parent-facing subset still follows in `ConfigSync`.

The `ConfigSync` payload currently includes:

- `Config` containing release-owned `version` metadata plus `CompanionDeviceInformation`, `httpClient`, and `UserInterface`; Companion Device-local `pinMode` is excluded
- `Board.Username`
- `Board.Password`
- `Board.ProductPlatform`
- `TransactionId`
...
| `ParentReadyRequest` | Companion Device to Parent Room Device | Implemented | The Companion Device asks the freshly installed Parent Room runtime to confirm it is ready and provides return-path credentials plus the HTTPClient policy needed for the first callback. |
...
| `DeregisterRequest` | Companion Device to Parent Room Device | Implemented, idempotent | The Companion Device asks the Parent Room Device to purge its peripheral and remove this Companion Device's config, registration, and pairing-state records; it repeats the HTTPClient callback policy so a lost acknowledgement remains retryable after removal. |
