...
The published Preview `v0.1.2.51` implements Installer Parent Room Registration but predates Inventory and Deregistration. The current installer does not yet gate Complete Setup actions by selected-release support, so it can request Inventory from that Preview and wait for a result the runtime cannot emit. The accepted [Companion Installer Compatibility and Capabilities](installer-capabilities.md) design adds a baseline Installer Contract Version, an informational Tested Installer Version, and release-declared Installer Capabilities to prevent that mismatch without exact package-version binding. This contract is not enforced until its pending installer, manifest, verifier, and test changes are implemented together. See [ADR 0010](adr/0010-release-declared-installer-capabilities.md).

Before packaging, `npm run verify:release` checks that the Release Manifest exactly covers the eligible root macros, the runtime project version is synchronized across Main, Config, `config.version`, and RoomReference, every macro passes JavaScript syntax validation, paired `Title` and `Text` fields for RoomOS UserInterface Messages contain no newline characters, relative macro imports resolve to Release Manifest resources, Main still emits the initialization messages used by installer verification, and the deployable source retains the Installer Parent Room Registration, Inventory, and Deregistration action and terminal result identifiers. Installer tests and builds run the same Release Contract verification before generating the pinned source snapshot.
