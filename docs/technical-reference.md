...
Both Companion Device Deployment Configuration and the Parent Room runtime default `allowInsecureHTTPS` to `false`. Device certificates and host names therefore need a trusted chain in both directions unless a Device Administrator deliberately sets `config.httpClient.allowInsecureHTTPS` to `true`, accepting untrusted certificates for Custom Companion device-to-device requests.
...
| `DeregisterRequest` | Companion Device to Parent Room Device | Implemented, idempotent | The Companion Device asks the Parent Room Device to purge its peripheral and remove this Companion Device's config, registration, and pairing-state records. |
