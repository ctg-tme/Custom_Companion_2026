...
- Provide PIN-gated Companion Device Select access for Parent Room Registration and Parent Room Deregistration, listing registered Parent Room Devices, live-verifying a selected Parent Room Device before entering Paired mode, returning the Companion Device to Standalone, and managing PIN Mode from the Config page.
- Require an administrator-enabled RoomOS HTTPClient and use the observed device-wide trust posture for device-to-device communication without changing it.
...
- Memory storage: generated state owned by `Memory-Storage-Functions-V2`; this stores Companion Device Parent Room Device records, Pending Deregistration tombstones, Companion Device PIN Mode state, captured Standalone Paired Environment and standby preferences, Parent Room Registration records, and the Parent Room Device's last Companion-authoritative pairing states.
...
| --- | --- |
| `Custom-Campanion_1_Main_2026` | Developer-owned `projectVersion`, Companion Device entry, initialization order, selection transitions, Unhealthy handling, and cross-controller coordination. |
...
| `Custom-Campanion_4_UI_2026` | Access/hidden panel XML, custom access-panel icon application, PIN and status prompts, shared Companion Device alert ownership, widget state, and Companion WebWidget adapter. |
...
| `Custom-Campanion_7_RoomReference_2026` | Inactive Parent Room entry source; installed and activated on a Parent Room Device as `Custom-Campanion_Room_2026`, where it owns registration validation, the Registered Companion Devices Control Panel, and parent-facing pairing alerts. |
...
All 15 deployable source macros use the same metadata rules. `Date Created` records the file's first tracked source date, while `Revised` records its latest content revision. Every source-header version uses four numeric components. Main's header and declared `projectVersion`, the Config header, and the RoomReference header share the four-part Project Version; each helper or domain macro keeps its own implementation version. Main owns `projectVersion` as installation-source metadata; Config exports only the editable `config` object. Header-only maintenance does not change either version set. The four-component helper-header normalization completed at Project Version `0.1.2.64` was a one-time formatting and provenance migration, not a revision of implementation-unchanged helpers.
...
The static browser installer in [`installer/`](../installer/) deploys a selected release or the current Main Fork (Beta) snapshot to a Companion Device through JSXAPI. The root [`manifest.json`](../manifest.json) is the Release Manifest and remains authoritative for installable project macros, minimum RoomOS, supported product platforms, external dependencies, and Companion Installer compatibility. The Release page first presents the solution-wide device, HTTPClient, bidirectional network, account, serial, Macro Runtime, and maintenance prerequisites. Opening Release automatically prepares the default source, and changing the selection immediately prepares the newly selected source. Successful preparation keeps the Device Administrator on the compact Release page and adds its manifest-derived minimum RoomOS version, software and product platforms, and external macro dependencies before Connect; a failed load exposes an explicit retry. Main Fork (Beta) acknowledgement still gates continuation. Product comparison normalizes Cisco formatting and display-size variants, then checks the selected manifest exactly. When no exact match exists, a product containing `Desk` or `Board Pro` is accepted only when the selected release declares a product in that family. This fallback applies consistently to retained releases without requiring their manifests to be rewritten. A product outside those declared families remains blocked by default; a Device Administrator may explicitly acknowledge an unsupported-device exploration warning for the current browser session. Configure and Review retain that warning, and the bypass never changes the selected release or implies that its runtime is validated on the device. The selected pinned release also supplies the Config macro parsed by the installer, so ordinary configuration fields are naturally release-specific. Trailing comments beside literal Config values provide source-owned definitions that the installer displays beneath their controls; older releases without these comments remain supported. String fields named `userGuidance` use full-width multiline controls. The Project Version is shown from the selected installation source rather than presented as Deployment Configuration. Review omits internal Installer Contract metadata, avoids duplicating the detailed HTTPClient posture, and wraps long installation-type labels. The installer never targets a Parent Room Device directly; the installed Companion Device runtime retains Parent Room provisioning ownership. [Network Requirements](network-requirements.md) lists the installer endpoints and separates them from macro runtime connectivity.

After Companion Device configuration and before Review, the installer requires the Device Administrator to choose **Install or Update — Keep Saved Data** or **Fresh Installation — Erase Saved Data**. Install or Update works for a new endpoint or an upgrade and preserves `Custom-Campanion-Storage`. Fresh Installation deactivates the existing project macros, removes only `Custom-Campanion-Storage` when present, and then installs the selected release. This permanently resets the Companion Device's saved Parent Room Devices, Pending Deregistration tombstones, active Parent Room Device selection, PIN Mode state, and captured Standalone Paired Environment and standby preferences. Generated storage remains outside the Release Manifest and is never treated as a Legacy Project Macro.

The installer keeps at most one authenticated Companion Device JSXAPI session. While connected, the Connect fields and installation source are locked. Connect tells the Device Administrator to set `HttpClient Mode=On` before sign-in and to set `AllowInsecureHTTPS=True` when trusted, host-matching endpoint certificates are not provisioned. Every device-specific configuration recommendation links to `https://{host}/web/configurations?s={encoded search}` using the entered Companion or Parent host. During signed-in preflight the installer reads both values without changing them. A Mode other than `On` stops before any device mutation with `Set xConfiguration HttpClient Mode to On, then reconnect.` After preflight, the installer registers one `HttpClient AllowInsecureHTTPS` configuration subscription, performs another current read after that registration is active, and updates the displayed **HTTPClient Trust Posture** on Connect, Configure, Review, and the Add Parent modal for the remainder of the session. Disconnect and reset remove the subscription. Strict certificate validation (`False`) is the green recommended production posture and explains callback-host/SAN and issuing-CA requirements. Permissive validation (`True`) is a yellow warning-state lab posture. The Add Parent modal repeats the bidirectional requirements and supplies direct configuration links for both devices. This RoomOS device-to-device trust policy is distinct from the browser's WSS certificate trust needed to establish the installer session.

Confirmed Disconnect closes that session and clears credentials, identity confirmation, installed-macro inventory, and generated configuration while preserving the selected release snapshot. Completed progress steps are clickable before Install begins; backward navigation preserves entered values, while later forward movement repeats the normal validation. Configure can copy browser-provided Installer Computer coordinates into Weather and its IANA time zone into Time; both operations update the in-memory Config once and add no runtime xAPI surface. HTTP and HTTPS `iconUrl` values render a browser-side preview without changing URL ownership or reachability requirements. Navigation locks at Install. The installer subscribes to the macro log, saves and activates the selected source, explicitly issues `Macros Runtime Restart`, and then waits for the initialization result.

After a successful initialization, Complete Setup keeps that session open. It recommends Parent Room Registration from the Companion Device interface and includes an on-device walkthrough. With the current Main Fork runtime, Install or Update requests a credential-free **Installer Parent Room Inventory** showing registered and Pending Deregistration state. Fresh Installation instead starts from a known-empty inventory because it just erased `Custom-Campanion-Storage`; no initial inventory request or error is needed. A later explicit Refresh and post-registration refresh still request current runtime state. **Add Parent** starts Installer Parent Room Registration. A confirmed Remove starts **Installer Parent Room Deregistration**, which reuses the Companion Device's existing Standalone transition, durable local retirement, remote cleanup, tombstone, and retry workflow. The current runtime emits transaction-correlated progress for six registration stages and the applicable two or three deregistration stages; those events are additive, and existing terminal results remain authoritative for retained runtimes that do not emit progress. Initial registration transport failures retain the requested Parent host, current stage, stable code, and certificate/HTTPClient remediation. Each browser attempt sends one local, transaction-correlated `Message.Send` request to the Companion Device; the installer never connects to or directly changes the Parent Room Device.

The Release Manifest declares a baseline Installer Contract Version, an informational Tested Installer Version, and versioned Installer Capabilities. Complete Setup renders and sends only workflows declared by the selected release, and every action rechecks its capability. Unknown capabilities are ignored, while an absent known capability fails closed. The published Preview `v0.1.2.51` predates this metadata, so the installer assigns Registration-only support only when that exact tag resolves to its recorded immutable commit; Inventory is not requested and Deregistration is not exposed. Other metadata-free manifests and unsupported Contract Versions fail preflight. See [Companion Installer Compatibility and Capabilities](installer-capabilities.md) and [ADR 0010](adr/0010-release-declared-installer-capabilities.md).

Before packaging, `npm run verify:release` checks that the Release Manifest exactly covers the eligible root macros, every listed macro header has four numeric version components, and the runtime Project Version is synchronized across Main's header and declaration plus the Config and RoomReference headers. Acorn syntax analysis requires exactly one top-level Main `projectVersion` declaration and rejects an actual Config declaration or export without treating comments, strings, or nested Main declarations as ownership. Every macro also passes JavaScript syntax validation, paired `Title` and `Text` fields for RoomOS UserInterface Messages contain no newline characters, relative macro imports resolve to Release Manifest resources, and Main still emits the initialization messages used by installer verification. The verifier also requires the manifest Installer Contract Version and sorted capability catalog to match `installer/release-contract.json`, the Tested Installer Version to match `installer/package.json`, every declared capability dependency to be present, and every declared capability's action and terminal results to remain in deployable source. Installer tests and builds run the same Release Contract verification before generating the pinned source snapshot.

See [`installer/README.md`](../installer/README.md) for local commands and ADR 0002 through ADR 0005 plus ADR 0008 through ADR 0011 for source selection, credentials, Companion Device Identity Confirmation, forward-only installation, Installer Parent Room administration, release-declared compatibility, and administrator-owned HTTPClient trust decisions.

## Initialization

The Companion Device initializes first. Its first asynchronous prerequisite reads local `xConfiguration HttpClient Mode` and requires `On`, then observes `HttpClient AllowInsecureHTTPS`; neither configuration is ever set by the runtime. Only after that gate succeeds does Main initialize the in-memory transport queue, register UI routes, verify `CompanionDeviceInformation.host`, `.username`, and `.password`, initialize MemoryStorage, load local state including PIN Mode and Pending Deregistrations, register call/media subscriptions, validate known Parent Room Devices, apply local mode policy, install the Parent Room runtime package, connect itself as a peripheral, ask each online Parent Room Device to confirm that the runtime is ready before sending Companion Device-owned configuration, and make one cleanup attempt for each tombstone. The Parent Room entry macro performs the same local HTTPClient gate before MemoryStorage, message subscriptions, registration validation, standby, or call coordination.

The callback check confirms every required Companion Device Callback Credentials field is present without logging its value; the Companion Installer separately authenticates those credentials before installation. Missing callback configuration, HTTPClient failure, MemoryStorage failure, or PIN Mode initialization failure stops initialization, logs a stable administrator diagnostic, removes `cc26_access`, `cc26_hidden`, and legacy `cc26`, and installs the gray widgetless `cc26_error` action panel. A Companion Device Mode failure uses `CC26-INIT-HTTPCLIENT-MODE`, does not start a network-dependent controller or timer, and does not retry initialization or restart the Macro Runtime automatically.

```mermaid
flowchart TD
	A[Custom-Campanion_1_Main_2026 starts] --> B[Read local HttpClient Mode and trust posture]
	B -- Mode unavailable or not On --> X[Log diagnostic and install cc26_error]
	B --> C[Initialize in-memory HTTP transport]
	C --> D[Register UI handlers and verify callback credentials]
	D -- Missing or invalid --> X
...
| `standaloneUiFeatureConfig` | Captured during a Standalone initialization, immediately before entering Paired, and from supported configuration or WebWidget changes observed while Standalone. It also holds optional original WebWidget restore metadata. | Restores the exact supported Standalone UI feature values. |
...
The Parent Room Device has separate generated memory. `registeredBoards` stores its recognized Companion Device records, `boardConfigs` stores the last accepted Companion Device configuration by serial, and `companionPairingStates` stores the last Companion-authoritative `Paired` or `NotPaired` state by serial. `ConfigSync` writes the registration/configuration records and includes the current pairing state; confirmed deregistration removes all three records. Parent Room initialization reloads them before validation and call coordination begin. `Offline` is a current validation result rather than a replacement for the last authoritative pairing state.
...
Install or Update — Keep Saved Data preserves all of these records. Fresh Installation — Erase Saved Data deliberately removes the generated Companion Device storage and therefore removes both the operating-mode authority and every Standalone Preference Snapshot. Because RoomOS configurations themselves persist, a Fresh Installation performed while Paired can leave Paired-enforced configuration values on the device; the next initialization has no deleted snapshot from which to recover the earlier Standalone values.
...
The **HTTPClient Trust Posture** is administrator-owned and device-wide. Both runtime entry macros require local `HttpClient Mode=On`, observe `HttpClient AllowInsecureHTTPS`, and never change either configuration. Every request explicitly supplies `AllowInsecureHTTPS=True`; RoomOS still requires valid certificate trust when the sending device's device-wide setting is `False`. The production posture is `Mode=On` and `AllowInsecureHTTPS=False`, with each sending device trusting the remote issuing CA and using a host name or IP address present in the remote certificate SAN. The lab posture is `Mode=On` and `AllowInsecureHTTPS=True`, which permits untrusted or self-signed remote certificates for every qualifying HTTPClient destination on that sending device.

The current Config has no HTTPClient policy field, and no trust policy is sent in `ParentReadyRequest`, `ConfigSync`, `DeregisterRequest`, or call/standby traffic. Older selected releases remain editable through the installer's source-driven Config parser. Legacy `boardConfigs[*].httpClient` values can remain stored after an update, but the current Parent Room runtime ignores them. A mixed per-Companion exception model is intentionally unsupported because RoomOS owns the effective certificate-validation gate per sending device, not per registered destination.

The exact `/getxml` and `/putxml` endpoint patterns are listed under Macro Core Needs in [Network Requirements](network-requirements.md).
...
The envelope is intentionally small because RoomOS `Message.Send` text is limited to 8192 characters. Timestamps are left to the macro logs, serial data is only sent once at the top level, and empty `Source` fields are omitted. Registration messages carry a `Payload.TransactionId` so stale acknowledgements cannot reverse newer intent. `ParentReadyRequest`, `ConfigSync`, and idempotent `DeregisterRequest` are intentionally allowed before the Parent Room Device recognizes the Companion Device serial. `PairingStatus` and the pairing state returned by `RegistrationValidated` are accepted only from a serial already present in the Parent Room Device's `registeredBoards` memory list.

The legacy `App`, `Source.Role`, `Board`, `registeredBoards`, `boardConfigs`, capacity/reconciliation payload fields, diagnostic codes, macro filenames, `board.joinCall` route, and `board-device` lab alias remain compatibility identifiers. The current configuration node and newly emitted `ConfigSync` field use `CompanionDeviceInformation`; the Companion Installer still recognizes the older `CompanionBoardInformation` Config node when installing an older selected release. Product names such as Board Pro and xAPI paths such as `Whiteboard` also remain exact. Human-facing UI, documentation, and logs use Companion Device terminology.
...
Companion Device serial, name, and MAC address are not stored in base config. The Companion Device pulls those values from local xAPI at runtime and places them in the message envelope when needed. `ParentReadyRequest` contains return credentials but no deployment configuration; the explicit Parent-facing subset follows in `ConfigSync`.

The `ConfigSync` payload currently includes:

- `Config` containing release-owned `version` metadata plus `CompanionDeviceInformation` and `UserInterface`; Companion Device-local `pinMode` and HTTPClient trust policy are excluded
- `Board.Username`
- `Board.Password`
- `Board.ProductPlatform`
- `TransactionId`
...
The Companion Device is authoritative for whether it is Paired to a particular Parent Room Device. A completed Parent Room transition publishes `PairingStatus=Paired`; a transition to Standalone or to a different Parent Room Device publishes `PairingStatus=NotPaired` to the previous parent. Companion Device initialization and later parent-availability refreshes silently synchronize the state to each online Parent Room Device. The parent persists only changed authoritative states, updates the `Registered Companion Devices` panel in `ControlPanel`, and uses the same downloaded Custom Companion icon as Companion Device Select.

The panel always begins with an explanatory row. Each registered Companion Device has its own row name and a read-only `Paired`, `Not paired`, or `Offline` text value. With no registrations, row 2 reads `Registration Status` / `No Companion Devices Registered`; the panel remains installed for later registration.

On Parent Room runtime initialization, validation is asynchronous but serialized across Companion Devices. Each registered Companion Device receives up to three `RegistrationValidation` requests; a successfully sent request has a two-second response window before the next attempt. A current `RegistrationValidated` response returns authoritative pairing state; a valid legacy response without that field retains the saved state while still proving reachability. A matching stored state updates reachability without an alert; a changed state is persisted and becomes an alert candidate. Three unanswered attempts mark that Companion Device `Offline` without overwriting the last authoritative pairing state.

Startup waits for all registered Companion Device validation sequences before rendering the final panel and may show at most one 10-second `Alert.Display`. Relevance is `Paired` change, then `NotPaired` change, then `Offline`; names tied at the highest priority are concatenated in the single alert. Later authoritative messages immediately replace `Offline`, update persistence when needed, and show a transition alert only when Paired/Not paired differs from the saved state.

`DeregisterRequest` is accepted whether or not the Companion Device serial is still present, so a lost acknowledgement can be retried safely. It carries return credentials but no HTTPClient trust policy; the Parent Room Device's local administrator-owned posture remains authoritative after the stored record has already been removed by an earlier attempt. The Parent Room Device checks `Status.Peripherals.ConnectedDevice`; when the Companion Device peripheral exists it invokes `Peripherals.Purge` once, and an absent entry is already complete. It then removes the serial from `boardConfigs`, `registeredBoards`, and `companionPairingStates`, persists all three, updates Parent Call Coordination and the status panel, and sends the transaction-correlated `DeregistrationAccepted`. The installed Parent Room macro package remains active for other registered Companion Devices.
...
After Install or Update, Complete Setup sends `InstallerParentInventoryRequest` to the authenticated Companion Device and waits for the matching macro-log result. The returned Registered list contains name, host, serial, and active status. The Pending Deregistrations list also contains tombstone creation time. Stored Parent Room Device usernames and passwords are never returned to the browser. Fresh Installation already establishes that both lists are empty by erasing generated storage, so its first Complete Setup render skips this request and identifies the state as known-empty.

Complete Setup offers an optional **Add Parent** modal. A Device Administrator may use it zero or more times instead of the in-room wizard, or leave Parent Room Registration for the Companion Device interface. The browser form collects the Parent Room Device host, expected serial, and credentials, then sends `InstallerParentRegistrationRequest`. The Companion Device verifies that the request names itself, performs the same live Parent Room Device serial confirmation and provisioning stages, reports additive transaction-correlated stage progress when supported, and finishes with the existing transaction-correlated terminal result. The in-room UI remains unchanged and PIN Mode is not displayed or requested for this Device Administrator workflow. If the verified Parent Room Device is already registered or has a Pending Deregistration, the installer requires an explicit replacement acknowledgement before the Companion Device makes the new registration the current intent.

Removing a registered row requires browser confirmation and sends `InstallerParentDeregistrationRequest` with the selected serial. The Companion Device runs the same deregistration operation used by the in-room workflow. An active selection first returns to Standalone and leaves any active Companion Device call; local retirement creates a Pending Deregistration tombstone before remote cleanup begins. A confirmed remote acknowledgement reports completion. An unreachable Parent Room Device reports a terminal pending result, disappears from Registered, and remains visible under Pending Deregistrations while the Companion Device retries. If the browser's first monitoring window expires before a terminal result, the modal offers **Keep Waiting** and retains the original transaction-correlated macro-log subscription; continuing does not resend the local `Message.Send` command or create another subscription. Completed, Pending, and Failed results are terminal, while cancellation, reset, or disconnect closes the retained monitor. The browser does not edit `Custom-Campanion-Storage`, request PIN Mode, or directly control the Parent Room Device.
...
The locked stages authenticate to the host, read its live identity, and require the observed serial to match the entered serial before any Parent Room macro changes. A mismatch reports failure without displaying the observed serial. After identity confirmation, the workflow asks before replacing an existing serial or canceling that serial's Pending Deregistration, installs/starts the shared Parent Room macros, connects and heartbeats the Companion Device peripheral, waits for `ParentReady`, waits for `ConfigAccepted`, and finally saves the Parent Room Device record. `ParentReadyRequest` and `ConfigSync` retry every five seconds only inside their respective 60-second stages. Installer-originated terminal failures identify the stage, stable code, and requested or verified Parent Room Device host without exposing credentials. A failure during the initial HTTPS read explains that the Companion Device could not reach or authenticate to that host and directs the Device Administrator to Mode, network, credentials, CA trust, SAN matching, or deliberately permissive lab posture as applicable. The candidate credentials stay transient until the Parent Room Device accepts and the Companion Device storage write succeeds. A new serial is rejected when the Companion Device already has six registered Parent Room Devices; a Parent Room Device rejects a new Companion Device when it already has three. Registration is blocked only while the Companion Device is Paired and in an active call. A Standalone call does not block it.

If the verified serial already exists, the Companion Device asks whether to overwrite the saved name, host, and credentials. If the serial has a Pending Deregistration tombstone, it asks whether the user wants to make registration the newer intent. Decline keeps the existing state. For an installer request, the submitted replacement acknowledgement suppresses that serial's older cleanup from initial identity verification onward; for the in-room route, suppression begins after the verified user decision. This prevents the older removal intent from racing the new registration handshake. Only `ConfigAccepted` plus the local registration write replaces the old record or tombstone, and any failed attempt releases suppression so Pending Deregistration retries resume. A failure does not create a selectable Parent Room Registration. If configuration may already have reached the Parent, the Companion Device retains only a hidden cleanup tombstone and tells the user to inspect macro logs.
...
The hidden `pendingDeregistrations` record retains the Parent Room Device serial and connection data until a matching `DeregistrationAccepted` transaction arrives. The Parent Room Device purges the Companion Device's `Peripherals ConnectedDevice` entry, removes `registeredBoards`, `boardConfigs`, and `companionPairingStates`, and only then acknowledges. An already-absent peripheral counts as complete. Cleanup is attempted during the locked Parent Room Deregistration stage, at Companion Device initialization, and whenever a valid message arrives from that pending Parent Room Device. Parent Room Device initialization sends `RegistrationValidation` to its saved Companion Devices; an active registration replies `RegistrationValidated`, while a Pending Deregistration immediately retries `DeregisterRequest`. A response from the expected serial with the expected transaction and `Registered` status proves reachability. Current responses also supply authoritative `Paired` or `NotPaired` state; a valid legacy response that omits `PairingState` retains the Parent's last saved state, while an explicitly invalid supplied state is rejected. Unknown serials, mismatched transactions, malformed responses, and absent responses retain the existing ignore, retry, and Offline behavior. Messages from unknown or retired Parent Room Devices are otherwise ignored. See [ADR 0006](adr/0006-parent-registration-and-tombstone-reconciliation.md).
...
`config.UserInterface.WebWidget.CompanionWidget.enabled` is `true` by default and is the sole control for the managed Companion WebWidget workflow. When it is `false`, initialization makes one best-effort removal attempt for the exact solution-owned `cc26WebWidget` and does not inventory, capture, build, save, restore, read or subscribe to Theme, or register `LayoutUpdated`; a differently owned WebWidget is left untouched and unrelated Paired Environment subscriptions remain active. A failed optional removal is reported once without retrying or entering Unhealthy State. When enabled, the UI adapter inventories the installed WebWidget with `UserInterface.Extensions.List({ ActivityType: 'WebWidget' })` and recursively locates the returned `WebWidget` extension metadata. In that command response, RoomOS supplies the saved WebWidget URL as `ActivityData`; the adapter normalizes it with the `PanelId`, name, and refresh interval. Because RoomOS supports only one WebWidget, replacing one whose `PanelId` differs from the intended widget first removes that exact current `PanelId` with `UserInterface.Extensions.WebWidget.Remove`, then saves the intended definition.

By default, `restoreStandaloneExisting` is `false`: any pre-existing WebWidget is replaced without being retained, and the Companion Widget is shown in both Standalone and Paired modes. When `restoreStandaloneExisting` is `true`, a valid non-Companion WebWidget discovered while the operating mode is known to be Standalone is saved as a durable Standalone Preference Snapshot containing its exact URL, `PanelId`, name, and refresh interval. `Event.UserInterface.Extensions.Widget.LayoutUpdated` processing is serialized. Each event verifies Standalone before inventory, after inventory, and immediately before persistence; it ignores unchanged, empty, malformed, Paired-observed, and solution-owned definitions. A changed definition is written as a candidate snapshot before it becomes authoritative in controller memory or produces a success log, so a failed write retains the previous preference and the same later event retries. Entering Paired removes only the durably captured Standalone widget and saves `cc26WebWidget`; returning to Standalone removes `cc26WebWidget` and restores the latest durable definition. When management is re-enabled in Standalone, initialization inventories the current user-owned WebWidget even when an older snapshot exists and publishes the newer definition only after persistence succeeds. Re-enabling while Paired does not learn a Standalone value. An empty or malformed WebWidget restore record is discarded so a later Standalone capture can relearn a valid original; it cannot reconstruct an original that RoomOS has already removed.

The Companion Widget always uses the release-owned Simple-WebWidget URL with hash parameters. `Standalone.iconUrl` and `Paired.iconUrl` independently supply the WebWidget image for their respective modes and are both prefilled with the solution-owned icon URL. Weather and time are disabled by default, with blank location and time-zone values. Other configurable CompanionWidget fields include weather.mode, weather.latitude, weather.longitude, weather.temperatureUnit, time.mode, time.timeZone, `Standalone.userGuidance`, and `Paired.userGuidance`. The UI adapter maps the selected mode's `userGuidance` to Simple-WebWidget's `info2` hash parameter. The Companion Device supplies theme, heading, `info1`, solution-owned runtime `info3`, and `hideSettings=true` in code, and re-saves the widget if `UserInterface Theme Name` changes. [Network Requirements](network-requirements.md) groups the WebWidget host and its downstream services under Simple-WebWidget.

Companion Device Select UI construction saves `cc26_access` and `cc26_hidden` first, then dispatches the release-owned panel-icon download before selected-parent or PIN widget feedback. It applies the returned `IconId` to `cc26_access` through `UserInterface.Extensions.Panel.Update({ Icon: 'Custom' })`. The panel icon URL is a UI-module constant and is not affected by either WebWidget `iconUrl` field. A successful download is reused for later UI rebuilds during that Macro Runtime session. Each build makes at most one download attempt; an unavailable icon leaves the built-in `Input` icon in place and is reported through the existing soft UI-render diagnostic rather than creating an Unhealthy State. The release-owned source image is [`assets/icons/custom-companion-512.png`](../assets/icons/custom-companion-512.png).
...
Required local prerequisite failures, invalid saved PIN Mode state, and required Paired microphone/volume/DND enforcement failures enter an Unhealthy State. The console logs a stable code, component, context, remediation, and original error without credentials or PIN values. The `cc26_access` and `cc26_hidden` panels are replaced by the gray widgetless `cc26_error` action panel, blocking Parent Room Device selection. Clicking it shows `Contact a Device Administrator.` for up to 30 seconds with a Dismiss option.

When the managed Companion WebWidget is available, solution-owned Infoblock 3 persistently shows `Companion Device controls are unavailable. Contact a Device Administrator.` for general Unhealthy failures. A local HTTPClient Mode failure instead shows `Custom Companion is unavailable because device-to-device communication is disabled. Ask a Device Administrator to enable HTTPClient Mode and restart the Macro Runtime.` The cause-specific message has the same Unhealthy precedence over connectivity, call, and standby status and deliberately bypasses the normal dynamic-text length cap so the administrator action is preserved. A missing or unavailable WebWidget is logged without raising another hard error. Recovery requires correcting the local macro/xAPI issue and restarting the Macro Runtime; no background initialization retry or self-restart is attempted.
...
| --- | --- | --- |
| HTTPClient startup prerequisite | Companion and Parent entry macros read `xapi.Config.HttpClient.Mode.get()` and `xapi.Config.HttpClient.AllowInsecureHTTPS.get()` before network-dependent initialization; neither path calls `.set()` | None; a changed posture takes effect only after a Device Administrator restarts the Macro Runtime |
...
| Managed Companion WebWidget | When enabled, `xapi.Command.UserInterface.Extensions.List({ ActivityType: 'WebWidget' })` plus exact `WebWidget.Save/Remove` commands; when disabled, one initialization removal targets only `cc26WebWidget` | When enabled with Standalone restoration, `xapi.Event.UserInterface.Extensions.Widget.LayoutUpdated.on(...)`; only changed valid non-Companion definitions observed while Standalone replace the saved preference |
...
| Parent pairing status UI | Parent-local `xapi.Command.UserInterface.Extensions.Panel.Save(...)`, `Icon.Download(...)`, `Panel.Update(...)`, and `UserInterface.Message.Alert.Display(...)` | Read-only Control Panel; pairing changes arrive through `xapi.Event.Message.Send.on(...)` |
...
| `InstallerParentInventoryRequest` | Companion Installer to Companion Device | Implemented, Device Administrator workflow | A local browser JSXAPI `Message.Send` request returns transaction-correlated sanitized Parent Room Registrations and Pending Deregistrations without stored credentials. |
| `InstallerParentDeregistrationRequest` | Companion Installer to Companion Device | Implemented, Device Administrator workflow | A confirmed browser request starts the Companion Device-owned Parent Room Deregistration pipeline without rendering the in-room PIN or progress prompts. The existing Standalone transition, local tombstone, remote cleanup retry, and acknowledgement semantics remain authoritative. |
| `ParentReadyRequest` | Companion Device to Parent Room Device | Implemented | The Companion Device asks the freshly installed Parent Room runtime to confirm it is ready and provides return-path credentials. The Parent Room Device's local administrator-owned HTTPClient Trust Posture governs the callback. |
...
| `ConfigSync` | Companion Device to Parent Room Device | Implemented | The Companion Device sends the explicit Parent Room-facing config subset, return credentials, capabilities, and its current pairing state for that parent. Companion Device-local `pinMode` is excluded. |
...
| `RegistrationValidation` | Parent Room Device to Companion Device | Implemented | Parent Room runtime initialization asks each saved Companion Device to prove whether the Parent Room Registration and saved pairing state are still current. Validation is serialized and uses three attempts with a two-second response delay. |
| `RegistrationValidated` | Companion Device to Parent Room Device | Implemented | A Companion Device with an active registration confirms the Parent Room Device record. Current responses return authoritative pairing state; a transaction-correlated legacy response that omits `PairingState` proves reachability while retaining the Parent's saved `Paired` or `NotPaired` state. A tombstoned Companion Device sends deregistration instead. |
| `PairingStatus` | Companion Device to Parent Room Device | Implemented | A registered Companion Device publishes its authoritative `Paired` or `NotPaired` state after transitions and silently during later synchronization. The parent compares before persisting, updating its panel, or displaying a 10-second transition alert. |
| `DeregisterRequest` | Companion Device to Parent Room Device | Implemented, idempotent | The Companion Device asks the Parent Room Device to purge its peripheral and remove this Companion Device's config, registration, and pairing-state records. Return credentials remain in the request so a lost acknowledgement is retryable after removal; no HTTPClient policy is handed off. |
| `DeregistrationAccepted` | Parent Room Device to Companion Device | Implemented | The Parent Room Device confirms peripheral and stored-record cleanup; the matching transaction removes the Companion Device tombstone. |
...
- `companionPairingStates` retains the last Companion-authoritative pairing state by registered Companion Device serial; transient `Offline` display state does not overwrite it.
...
`Custom-Campanion-Storage.js` is generated database state managed by the memory storage library and should not be edited or committed as source. The Companion Installer preserves it during Install or Update — Keep Saved Data and removes it only when a Device Administrator explicitly selects Fresh Installation — Erase Saved Data.
