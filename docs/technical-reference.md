...
| `Custom-Campanion_2_Config_2026` | Developer-owned `projectVersion` plus Deployment Configuration, including first-initialization `pinMode.defaults`. |
...
All 15 deployable source macros use the same metadata rules. `Date Created` records the file's first tracked source date, while `Revised` records its latest content revision. Main, Config, the Config macro's exported `projectVersion`, and RoomReference share the four-part Project Version; each helper or domain macro keeps its own implementation version. `projectVersion` is imported by Main as a developer-owned dependency and is not part of the editable `config` object. Header-only maintenance does not change either version set.
