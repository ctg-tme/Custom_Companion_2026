...
All 15 deployable source macros use the same metadata rules. `Date Created` records the file's first tracked source date, while `Revised` records its latest content revision. Every source-header version uses four numeric components. Main's header and declared `projectVersion`, the Config header, and the RoomReference header share the four-part Project Version; each helper or domain macro keeps its own implementation version. Main owns `projectVersion` as installation-source metadata; Config exports only the editable `config` object. Header-only maintenance does not change either version set.
...
On Parent Room runtime initialization, validation is asynchronous but serialized across Companion Devices. Each registered Companion Device receives up to three `RegistrationValidation` requests; a successfully sent request has a two-second response window before the next attempt. `RegistrationValidated` returns the current pairing state. A matching stored state updates reachability without an alert; a changed state is persisted and becomes an alert candidate. Three unanswered attempts mark that Companion Device `Offline` without overwriting the last authoritative pairing state.
...
`config.UserInterface.WebWidget.CompanionWidget.enabled` is `true` by default and is the sole control for the managed Companion WebWidget workflow. When it is `false`, initialization makes one exact removal attempt for the solution-owned `cc26WebWidget` and does not inventory, capture, restore, subscribe to, build, or update WebWidgets; a differently owned WebWidget is left untouched. When enabled, the UI adapter inventories the installed WebWidget with `UserInterface.Extensions.List({ ActivityType: 'WebWidget' })` and recursively locates the returned `WebWidget` extension metadata. In that command response, RoomOS supplies the saved WebWidget URL as `ActivityData`; the adapter normalizes it with the `PanelId`, name, and refresh interval. Because RoomOS supports only one WebWidget, replacing one whose `PanelId` differs from the intended widget first removes that exact current `PanelId` with `UserInterface.Extensions.WebWidget.Remove`, then saves the intended definition.

By default, `restoreStandaloneExisting` is `false`: any pre-existing WebWidget is replaced without being retained, and the Companion Widget is shown in both Standalone and Paired modes. When `restoreStandaloneExisting` is `true`, a non-Companion WebWidget with a non-empty normalized URL discovered while the operating mode is known to be Standalone is saved as a durable Standalone Preference Snapshot containing its URL, `PanelId`, name, and refresh interval. `Event.UserInterface.Extensions.Widget.LayoutUpdated` causes the controller to re-inventory the installed WebWidget with `Extensions.List`; a changed valid non-Companion definition observed while Standalone replaces the saved snapshot. Entering Paired removes that exact Standalone widget and saves `cc26WebWidget`; returning to Standalone removes `cc26WebWidget` and restores the latest captured definition. Layout changes observed while Paired and the solution-owned `cc26WebWidget` are never learned as Standalone preferences. An empty or malformed WebWidget restore record is discarded so a later Standalone capture can relearn a valid original; it cannot reconstruct an original that RoomOS has already removed.
...
| `RegistrationValidated` | Companion Device to Parent Room Device | Implemented | A Companion Device with an active registration confirms the Parent Room Device record and returns its authoritative pairing state for that parent; a tombstoned Companion Device sends deregistration instead. |
