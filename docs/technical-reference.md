...
The Companion Widget uses the built-in Simple-WebWidget URL with hash parameters unless `config.UserInterface.WebWidget.urlOverride` is supplied. `Standalone.iconUrl` and `Paired.iconUrl` independently supply the WebWidget image for their respective modes and are both prefilled with the solution-owned icon URL. Weather and time are disabled by default, with blank location and time-zone values. Other configurable CompanionWidget fields include weather.mode, weather.latitude, weather.longitude, weather.temperatureUnit, time.mode, time.timeZone, `Standalone.userGuidance`, and `Paired.userGuidance`. The UI adapter maps the selected mode's `userGuidance` to Simple-WebWidget's `info2` hash parameter. The Companion Device supplies theme, heading, `info1`, solution-owned runtime `info3`, and `hideSettings=true` in code, and re-saves the widget if `UserInterface Theme Name` changes. [Network Requirements](network-requirements.md) groups the WebWidget host and its downstream services under Simple-WebWidget.
...
| Standalone WebWidget restoration | `xapi.Command.UserInterface.Extensions.List({ ActivityType: 'WebWidget' })` plus exact `WebWidget.Save/Remove` commands | `xapi.Event.UserInterface.Extensions.Widget.LayoutUpdated.on(...)`; only changed valid non-Companion definitions observed while Standalone replace the saved preference |
