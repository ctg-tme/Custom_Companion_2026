...
The installer keeps at most one authenticated Companion Device JSXAPI session. While connected, the Connect fields and installation source are locked. During signed-in preflight it reads `HttpClient Mode` and `HttpClient AllowInsecureHTTPS` without changing either value. A Mode other than `On` stops before any device mutation with `Set xConfiguration HttpClient Mode to On, then reconnect.` Configure and Review report the observed **HTTPClient Trust Posture**. Strict certificate validation warns that the configured Companion callback host must match the Companion Device certificate SAN and that Parent Room Devices must trust its issuing CA. This RoomOS device-to-device trust policy is distinct from the browser's WSS certificate trust needed to establish the installer session.
