...
After installation, Complete Setup sends `InstallerParentInventoryRequest` to the authenticated Companion Device and waits for the matching macro-log result. The returned Registered list contains name, host, serial, and active status. The Pending Deregistrations list also contains tombstone creation time. Stored Parent Room Device usernames and passwords are never returned to the browser.
