...
All 15 deployable source macros use the same metadata rules. `Date Created` records the file's first tracked source date, while `Revised` records its latest content revision. Main's header and declared `projectVersion`, the Config header, and the RoomReference header share the four-part Project Version; each helper or domain macro keeps its own implementation version. Main owns `projectVersion` as installation-source metadata; Config exports only the editable `config` object. Header-only maintenance does not change either version set.
