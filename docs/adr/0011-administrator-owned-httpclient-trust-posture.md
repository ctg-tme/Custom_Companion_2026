...
The Release and Connect pages state the `HttpClient Mode=On` prerequisite before sign-in and tell a Device Administrator to use `AllowInsecureHTTPS=True` when trusted, host-matching endpoint certificates are not provisioned. During signed-in Companion Device preflight, the installer reads Mode and AllowInsecureHTTPS without setting either. Mode other than `On` blocks before any device mutation with the exact action `Set xConfiguration HttpClient Mode to On, then reconnect.` After preflight, the installer subscribes to `HttpClient AllowInsecureHTTPS`, refreshes its current value after subscription registration, and keeps the displayed posture current until disconnect. Connect, Configure, Review, and Installer Parent Room Registration report the observed posture and explain the applicable callback-host/SAN, issuing-CA, or permissive device-wide requirements. The browser WSS certificate recovery path remains separate and no installer control changes HTTPClient configuration.
