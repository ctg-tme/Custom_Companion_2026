...
Signed-in Companion Device preflight reads `HttpClient Mode` and `HttpClient AllowInsecureHTTPS` without changing either value. Mode must be `On`; otherwise installation stops before any mutation with `Set xConfiguration HttpClient Mode to On, then reconnect.` Configure and Review report the administrator-owned, device-wide **HTTPClient Trust Posture**. Strict validation includes callback-host/SAN and Parent issuing-CA guidance. This check is independent of the browser WSS certificate trust needed to establish the JSXAPI session, and the installer offers no action that changes RoomOS HTTPClient configuration.
