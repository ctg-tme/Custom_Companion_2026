...
Installer and callback credentials are held only in browser memory. The callback account must already exist on the same Companion Device. The installer does not create accounts or connect directly to Parent Room Devices. After successful initialization, a Device Administrator may optionally submit Parent Room Device details through the installer; the Companion Device runs its existing registration workflow and the installer waits for its transaction-correlated success or failure result before disconnecting.
