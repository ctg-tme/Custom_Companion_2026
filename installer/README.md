...
After successful initialization, Complete Setup keeps the Device Administrator session connected. It shows sanitized Parent Room Registrations and Pending Deregistrations, offers optional Add Parent, and can ask the Companion Device to deregister an existing Parent after browser confirmation. Registration shows six Companion Device-owned stages; deregistration shows the applicable two or three stages depending on whether Standalone must be restored first. Current runtimes emit transaction-correlated progress while existing terminal results remain authoritative, so older retained releases can still complete without progress events. Initial HTTPS verification failures retain the requested Parent host, stage, code, and actionable HTTPClient/certificate guidance. The browser never reads stored Parent Room credentials or connects directly to a Parent Room Device. Because Fresh Installation has just erased the authoritative generated storage, its initial Complete Setup view is known to contain no registrations and does not send a redundant inventory request. Finish explicitly disconnects the session.

Configure displays each current release Config definition from the trailing comment beside its source value. String fields named `userGuidance` use full-width multiline controls so the complete Standalone and Paired guidance remains visible while editing. Older selected releases without source definitions remain installable. The Review page presents the generated configuration with human-facing labels, including `Companion Device Information` and `Standalone`. Its operational summary omits internal Installer Contract metadata and does not duplicate the detailed HTTPClient posture panel; long installation-type labels wrap instead of truncating. The installer recognizes the older `CompanionBoardInformation` node only for compatibility with older selected releases; current generated Config uses `CompanionDeviceInformation`.
