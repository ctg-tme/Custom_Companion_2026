...
After a successful initialization, the same authenticated browser session can optionally start **Installer Parent Room Registration**. The installer sends one local, transaction-correlated `Message.Send` request to the Companion Device; it never connects to or changes the Parent Room Device itself. The Companion Device runs its registration pipeline and the installer reports only the Companion Device's terminal completion or failure result.
...
See `installer/README.md` for local commands and ADR 0002 through ADR 0005 plus ADR 0008 for source selection, credentials, Companion Device Identity Confirmation, forward-only installation, and Installer Parent Room Registration decisions.
