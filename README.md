...
| `CallSync` | Parent to board | Initial detection slice | Parent captures the first `Call RemoteNumber` value with a new-style one-shot status listener, pairs it with a new-style one-shot `CallSuccessful` listener, and sends the call details to registered boards. |
