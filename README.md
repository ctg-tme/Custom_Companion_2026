...
| `CallSync` | Parent to board | Initial detection slice | Parent captures the first non-empty indexed `Call[n].RemoteNumber` with new-style one-shot listeners, pairs it with a new-style one-shot `CallSuccessful` listener, and sends the call details to registered boards. |
