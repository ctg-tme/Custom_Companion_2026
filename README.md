...
Standalone standby preferences are saved in board memory for `Standby Control`, `Standby Halfwake Mode`, and `Time OfficeHours Enabled`. In paired mode, the board forces those values to `Off`, `Manual`, and `False` so it does not enter standby independently. When a parent is selected, the board reads that parent's current `Status.Standby.State` directly, and parent rooms also subscribe to `Status.Standby.State` to send debounced `StandbySync` messages to registered boards; a board only follows standby commands from its active paired parent. The board shows one 30-second prompt before applying a parent standby state, refreshes the prompt countdown in place, applies the latest valid state at the deadline, ignores `EnteringStandby`, and allows 5-minute or 30-minute bypass windows. While bypass is active, parent standby commands are ignored and the Web Widget `info3` shows `Standby sync bypass until HH:MM AM/PM`.
