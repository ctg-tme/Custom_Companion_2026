...
After a successful initialization, Complete Setup keeps the same authenticated browser session open. Its optional **Add Parent** action can start **Installer Parent Room Registration** zero or more times, or the Device Administrator can instead use the Companion Device interface. Each browser attempt sends one local, transaction-correlated `Message.Send` request to the Companion Device; the installer never connects to or changes the Parent Room Device itself. The Companion Device runs its registration pipeline and the installer reports only the matching terminal completion or failure result.
